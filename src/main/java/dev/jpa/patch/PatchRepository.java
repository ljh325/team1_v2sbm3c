...
public interface PatchRepository extends JpaRepository<Patch, Integer> {
  ...
  @Query(value="SELECT patchno, title, content, viewcnt, id, rdate FROM patch WHERE (SUBSTR(rdate, 1, 10) >= :start_date) AND (SUBSTR(rdate, 1, 10) <= :end_date)", nativeQuery = true)
  ...
  @Query(value="SELECT patchno, title, content, viewcnt, id, rdate FROM patch WHERE (SUBSTR(rdate, 1, 10) >= :start_date) AND (SUBSTR(rdate, 1, 10) <= :end_date) ORDER BY rdate DESC", nativeQuery = true)
  ...
  void increaseCnt(@Param("id") Integer id);
