...
public interface PatchRepository extends JpaRepository<Patch, Integer> {
  ...
  @Query(value="SELECT patchno, title, content, cnt, rdate FROM patch WHERE (SUBSTR(rdate, 1, 10) >= :start_date) AND (SUBSTR(rdate, 1, 10) <= :end_date)", nativeQuery = true)
  ...
  // Oracle 11G
  @Query(value="SELECT patchno, title, content, cnt, rdate, r FROM ( SELECT patchno, title, content, cnt, rdate, rownum as r FROM ( SELECT patchno, title, content, cnt, rdate FROM patch ORDER BY rdate DESC )) WHERE r <= 500", nativeQuery = true)
  List<Patch> findAllByOrderByRdateDesc();
  
  // SQL 사용, 컬럼을 모두 명시해야함, 날짜 구간 검색, 날짜 내림 차순 정렬
  @Query(value="SELECT patchno, title, content, cnt, rdate FROM patch WHERE (SUBSTR(rdate, 1, 10) >= :start_date) AND (SUBSTR(rdate, 1, 10) <= :end_date) ORDER BY rdate DESC", nativeQuery = true)
  List<Patch> findByRdateDescPeriod(@Param("start_date") String start_date, @Param("end_date") String end_date);

  // 조회수 증가
  @Modifying
  @Transactional
  @Query(value="UPDATE patch SET cnt = cnt + 1 WHERE patchno=:id", nativeQuery = true)
