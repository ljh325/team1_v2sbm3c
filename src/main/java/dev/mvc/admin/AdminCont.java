...
public class AdminCont {
  ...
  */
  ...
  public ModelAndView login_proc(
                            ...
                            HttpSession session,
                            AdminVO adminVO, String id_save, String password_save) {
    ModelAndView mav = new ModelAndView();
   
    int cnt = adminProc.login(adminVO);
    if (cnt == 1) { // 로그인 성공시 회원 정보 조회
      AdminVO adminVO_read = adminProc.read_by_adminid(adminVO.getAdminid()); // DBMS에서 id를 이용한 회원 조회
      session.setAttribute("adminno", adminVO_read.getAdminno()); // 서버의 메모리에 기록
      session.setAttribute("admin_id", adminVO_read.getAdminid());
      session.setAttribute("admin_grade", adminVO_read.getGrade());
   
      String adminid = adminVO.getAdminid();                  // 폼에 입력된 id
      String password = adminVO.getPassword();  // 폼에 입력된 passwd 
      ...
      // Password 관련 쿠기 저장
      // -------------------------------------------------------------------
      if (Tool.checkNull(password_save).equals("Y")) { // 패스워드 저장할 경우
        Cookie ck_admin_passwd = new Cookie("ck_admin_passwd", password);
      ...
      Cookie ck_admin_passwd_save = new Cookie("ck_admin_passwd_save", password_save);
