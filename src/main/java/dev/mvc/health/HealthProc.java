...
@Component("dev.mvc.health.HealthProc")
public class HealthProc implements HealthProcInter {
  @Autowired
  Security security;
  
  @Autowired // healthDAOInter interface를 구현한 클래스의 객체를 만들어 자동으로 할당해라.
  private HealthDAOInter healthDAO;
  

  @Override // 추상 메소드를 구현했음.
  public int create(HealthVO healthVO) {
    // -------------------------------------------------------------------
    String passwd = healthVO.getPasswd();
    String passwd_encoded = this.security.aesEncode(passwd);
    healthVO.setPasswd(passwd_encoded);
    // -------------------------------------------------------------------
    
    int cnt = this.healthDAO.create(healthVO);
    return cnt;
  }

  @Override
  public ArrayList<HealthVO> list_all() {
    ArrayList<HealthVO> list = this.healthDAO.list_all();
    return list;
  }

  @Override
  public ArrayList<HealthVO> list_by_htcno(int htcno) {
    ArrayList<HealthVO> list = this.healthDAO.list_by_htcno(htcno);
    return list;
  }
  ...
  public HealthVO read(int healthno) {
    HealthVO healthVO = this.healthDAO.read(healthno);
    return healthVO;
  }

  @Override
  public int map(HashMap<String, Object> map) {
    int cnt = this.healthDAO.map(map);
    return cnt;
  }

  @Override
  public int youtube(HashMap<String, Object> map) {
    int cnt = this.healthDAO.youtube(map);
    return cnt;
  }

  @Override
  public ArrayList<HealthVO> list_by_htcno_search(HashMap<String, Object> hashMap) {
    ArrayList<HealthVO> list = this.healthDAO.list_by_htcno_search(hashMap);
    return list;
  }

  @Override
  public int list_by_htcno_search_count(HashMap<String, Object> hashMap) {
    int cnt = this.healthDAO.list_by_htcno_search_count(hashMap);
    return cnt;
  }
  ...
  public ArrayList<HealthVO> list_by_htcno_search_paging(HashMap<String, Object> map) {
     ...
     */
    int begin_of_page = ((int)map.get("now_page") - 1) * Health.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 10 + 10: 20
    // 3 페이지 = 20 + 10: 30
    int end_num = begin_of_page + Health.RECORD_PER_PAGE;
    /*
    ...
    ArrayList<HealthVO> list = this.healthDAO.list_by_htcno_search_paging(map);
...
//  
//  <DIV id='paging'>
//    <span class='span_box_1'><A href='list_by_htcno.do?word=&now_page=1&htcno=4'>1</A></span>
//  <span class='span_box_2'>2</span>
//  <span class='span_box_1'><A href='list_by_htcno.do?word=&now_page=3&htcno=4'>3</A></span>
//    </DIV>
//  </DIV> 
//  <!-- 페이지 목록 출력 부분 종료 -->
   ...
   */
  @Override
  public String pagingBox(int htcno, int now_page, String word, String list_file, int search_count, int record_per_page,
      int page_per_block) {
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?htcno="+htcno+"&word=" + word + "&now_page=" + _now_page
          + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?htcno="+htcno+"&word=" + word + "&now_page=" + i + "'>" + i
            + "</A></span>");
      }
    }
  ...
  public int password_check(HashMap<String, Object> map) {
    ...
    int cnt = this.healthDAO.password_check(map);
    return cnt;
  }

  @Override
  public int update_text(HealthVO healthVO) {
    int cnt = this.healthDAO.update_text(healthVO);
    return cnt;
  }

  @Override
  public int update_file(HealthVO healthVO) {
    int cnt = this.healthDAO.update_file(healthVO);
    return cnt;
  }

  @Override
  public int delete(int healthno) {
    int cnt = this.healthDAO.delete(healthno);
    return cnt;
  }

  @Override
  public int count_by_htcno(int htcno) {
    int cnt = this.healthDAO.count_by_htcno(htcno);
    return cnt;
  }

  @Override
  public int delete_by_htcno(int htcno) {
    int cnt = this.healthDAO.delete_by_htcno(htcno);
    return cnt;
  }

  @Override
  public int count_by_adminsno(int adminsno) {
    int cnt = this.healthDAO.count_by_adminsno(adminsno);
    return cnt;
  }

  @Override
  public int delete_by_adminsno(int adminsno) {
    int cnt = this.healthDAO.delete_by_adminsno(adminsno);
    return cnt;
  }
