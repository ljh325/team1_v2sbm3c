...
//@Component("dev.mvc.htc.HtcProc")
@Service("dev.mvc.htc.HtcProc")
public class HtcProc implements HtcProcInter {
  @Autowired
  private HtcDAOInter htcDAO;
  
  public HtcProc() {
    // System.out.println("-> htcProc created.");  
  }
  
  @Override
  public int create(HtcVO htcVO) {
    int cnt = this.htcDAO.create(htcVO);
    return cnt;
  }

  @Override
  public ArrayList<HtcVO> list_all() {
    ArrayList<HtcVO> list = this.htcDAO.list_all();
    return list;
  }

  @Override
  public HtcVO read(int htcno) {
    HtcVO htcVO = this.htcDAO.read(htcno);
    return htcVO;
  }

  @Override
  public int update(HtcVO htcVO) {
    int cnt = this.htcDAO.update(htcVO);
    return cnt;
  }

  @Override
  public int delete(int htcno) {
    int cnt = this.htcDAO.delete(htcno);
    return cnt;
  }

  @Override
  public int update_seqno_forward(int htcno) {
    int cnt = this.htcDAO.update_seqno_forward(htcno);
    return cnt;
  }

  @Override
  public int update_seqno_backward(int htcno) {
    int cnt = this.htcDAO.update_seqno_backward(htcno);
    return cnt;
  ...
  public int update_visible_y(int htcno) {
    int cnt = this.htcDAO.update_visible_y(htcno);
    return cnt;
  }

  @Override
  public int update_visible_n(int htcno) {
    int cnt = this.htcDAO.update_visible_n(htcno);
    return cnt;
  }

  @Override
  public ArrayList<HtcVO> list_all_name_y() {
    ArrayList<HtcVO> list = this.htcDAO.list_all_name_y();
    return list;
  }

  @Override
  public ArrayList<HtcVO> list_all_namesub_y(String name) {
    ArrayList<HtcVO> list = this.list_all_namesub_y(name);
    return list;
  }

  @Override
  public ArrayList<HtcVOMenu> menu() {
    // 중분류 목록을 저장할 객체 선언
    ArrayList<HtcVOMenu> menu = new ArrayList<HtcVOMenu>();
    
    // 중분류 목록 로딩
    ArrayList<HtcVO> list = this.htcDAO.list_all_name_y();
    
    // 중분류 + 소분류 결합 
    for (HtcVO htcVO : list) {
      HtcVOMenu htcVOMenu = new HtcVOMenu(); // 하나의 중분류 메뉴 그룹
      
      htcVOMenu.setName(htcVO.getName()); // 중분류명
      
      // 중분류명을 이용하여 소분류 목록을 가져옴.
      ArrayList<HtcVO> list_namesub = this.htcDAO.list_all_namesub_y(htcVO.getName());
      htcVOMenu.setList_namesub(list_namesub);
  ...
  public ArrayList<HtcVO> list_search(String word) {
    ArrayList<HtcVO> list = this.htcDAO.list_search(word);
    return list;
  }

  @Override
  public ArrayList<HtcVO> list_search_paging(String word, int now_page, int record_per_page) {
    /*
    예) 페이지당 10개의 레코드 출력
    ...
    */
    ...
    ArrayList<HtcVO> list=this.htcDAO.list_search_paging(map);
    return list;
  }

  @Override
  public int list_search_count(String word) {
    int cnt = this.htcDAO.list_search_count(word);
    return cnt;
  }
