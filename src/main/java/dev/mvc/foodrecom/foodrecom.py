...
prompt = '''
    아래의 [회원 건강정보]를 바탕으로 [운동 목표]를 설정해서 운동중이야, [식단 목록]에서 운동 목표를 달성하기 위해 필요한 하루 식단을 출력해줘
    출력 형식은 JSON으로 해줘 
...
@app.get('/foodrecom/create')
def foodrecom_create_form():
    return render_template('foodrecom/create.html') # /templates/member_img.html

@app.post('/foodrecom/create')
def foodrecom_create_proc():
    
    mh = (80, 20, 180, 30)
    goals = (80, 20, 180, 32)
    filled_prompt = populate_prompt(prompt, mh, goals)
    print(filled_prompt)
    response = tool.answer(role='헬스트레이너야', prompt=filled_prompt, output='json', 
                        format='{"food": "값"}', llm='gpt-4o') 

    frecom = json.dumps(response)
